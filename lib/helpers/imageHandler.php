...
class imageHandler
{
	...
	public static function upload(array $imageData) {
		...
		extract(sanitize::array($imageData));
		...
		try {
			...
			if ($iName === NULL) {
				throw new RuntimeException($LANG["browse_img"], 1);
			}
			

			// check error value
			if ($err == TRUE) {
				throw new RuntimeException(uploadError::message($err));

			}

			// Validate Image authenticity
			if (getimagesize($tmpName) === FALSE) {
				throw new RuntimeException($LANG["invalid_img"], 1);
			...
			if ($fSize && $aSize) {
				...
				if ($imgAuthenticate == TRUE) {
					if ($fSize > $aSize) {
						throw new RuntimeException($LANG["img_size_limit"]." " .($aSize/1000000)."mb", 1);
			...
			switch ($fType) {
				...
				default            : 
					throw new RuntimeException($LANG["valid_img"], 1);
				  	$imgAuthenticate = FALSE;
				  	$ext = FALSE;
				  	//exit();
				  	break; 
